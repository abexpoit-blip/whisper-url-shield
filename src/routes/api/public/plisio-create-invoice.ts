...
const CreateInvoiceSchema = z.object({
  package_slug: z.string().trim().regex(/^[a-z0-9_-]{2,40}$/),
});
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          const baseAmount = Number(
            pkg.billing_period === "lifetime" || Number(pkg.price_onetime) > 0 ? pkg.price_onetime : pkg.price_monthly,
          );
          ...
          if (!res.ok || payload?.status !== "success" || !payload?.data?.invoice_url) {
            ...
            await (supabaseAdmin as any).from("upgrade_requests").update({ plisio_status: "error", note: message }).eq("id", reqRow.id);
            throw new Error(message);
          }

          await (supabaseAdmin as any)
            .from("upgrade_requests")
            .update({ plisio_invoice_id: payload.data.txn_id ?? null, plisio_invoice_url: payload.data.invoice_url })
            .eq("id", reqRow.id);
          ...
          await logActivity({
            ...
            metadata: { package_slug: body.package_slug, total_amount: totalAmount, duration_ms: Date.now() - startedAt, api_key_source: apiKeySource },
          });

          return Response.json({ invoice_url: payload.data.invoice_url, request_id: reqRow.id, total_amount: totalAmount });
        } catch (error) {
          ...
          await logActivity({ request_id: requestId, status_code: 400, outcome: "error", message, metadata: { duration_ms: Date.now() - startedAt } });

...
// Small inline SVGs — no external dependency, fully themable via `currentColor`
// where appropriate, but most brand marks use their official colors for instant
// recognition.
...
const wrap = (children: React.ReactNode, vb = "0 0 24 24") => (className?: string) => (
  <svg
...
export const AndroidIcon = ({ className }: IconProps) =>
  wrap(
    <path
      fill="#3DDC84"
      d="M17.523 15.341a1.005 1.005 0 1 1 0-2.01 1.005 1.005 0 0 1 0 2.01m-11.046 0a1.005 1.005 0 1 1 0-2.01 1.005 1.005 0 0 1 0 2.01m11.405-6.02 2.005-3.47a.418.418 0 0 0-.723-.418l-2.03 3.514a12.6 12.6 0 0 0-5.134-1.071c-1.823 0-3.557.388-5.135 1.07L4.836 5.434a.418.418 0 0 0-.723.418l2.005 3.47C2.69 11.196.4 14.738 0 18.92h24c-.4-4.182-2.69-7.724-6.118-9.598",
    />,
  )(className);
...
export const ChromeOSIcon = ({ className }: IconProps) =>
  wrap(
    <path
      fill="#4285F4"
      d="M12 0a12 12 0 0 1 10.39 6H12a6 6 0 0 0-5.2 9L1.61 6A12 12 0 0 1 12 0M3.34 7.5l5.2 9a6 6 0 0 0 10.4 0L13.74 24A12 12 0 0 1 3.34 7.5M22.66 16.5A12 12 0 0 1 12 24l5.2-9A6 6 0 0 0 12 6h10.39a12 12 0 0 1 .27 10.5",
    />,
  )(className);
...
export const FirefoxIcon = ({ className }: IconProps) =>
  wrap(
    <path
      fill="#FF7139"
      d="M9.79.2 8.86 1.96l.61 1.06.91.5L9.21 5.4l-1.78.42-.92 1.78-1.32 4.4 1.32 2.69 1.92 1.62 2.83.9 4.13-.91 2.96-2.96.92-3.43-1.5-3.13-2.95-1.5L12 2.74 9.79.2zM3.71 9.7 1.93 12l.5 3.7 2.66 3.46 4.27 2.83 4.84.9 4.41-.91 3.43-2.27 1.65-3.39-.41-3.74-2.74 2.83-3.21 1.65-3.86.42-2.83-1.5-1.78-2.27-.21-2.18.95-1.94-2.27.46L4.9 11.07 3.71 9.7z"
    />,
  )(className);
...
export const SafariIcon = ({ className }: IconProps) =>
  wrap(
    <>
      ...
      <path fill="#ED4231" d="m15 9-5 5 3-3 4-4-2 2z" opacity=".85" />
    </>,
  )(className);
...
export const FacebookIcon = ({ className }: IconProps) =>
  wrap(
    <path
      fill="#1877F2"
      d="M24 12c0-6.627-5.373-12-12-12S0 5.373 0 12c0 5.99 4.388 10.954 10.125 11.854V15.47H7.078V12h3.047V9.356c0-3.007 1.792-4.668 4.533-4.668 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.49 0-1.955.925-1.955 1.874V12h3.328l-.532 3.47h-2.796v8.385C19.612 22.954 24 17.99 24 12"
    />,
  )(className);
...
// Normalize keys so "Facebook/IG In-App", "iOS", "macOS" all match.
...
export function getBrandIcon(rawKey: string): React.ComponentType<IconProps> {
  ...
  if (k.includes("macos") || k.includes("macosx") || k === "mac") return AppleIcon;
  ...
  if (k.includes("instagram") || k.includes("ig")) return InstagramIcon;
...
export function prettyLabel(rawKey: string) {
  const k = norm(rawKey);
  if (k === "ios") return "iOS";
  if (k.includes("macosx") || k === "mac" || k.includes("macos")) return "macOS";
